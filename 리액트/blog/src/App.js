...
function App() {
  ...
  let [modal, setModal] = useState(true); // ui현재상태 : 열림, 닫힘, 보임, 1, true
  ...
  return (
    <div className="App">
        ...
        <button onClick={()=>{
          // 글제목 변경 => 여름 코트 추천, 역삼 우동 맛집, 리액트 독학
      ...
      {
        a.map(function(글, i){ // a: 순차적으로 접근한 요소 / i: 인덱스 : 0
          return (
            <div className="list">
        ...
        /*
          응용 1. 글에 아무것도 입력안해도 추가버튼 누르면 추가 잘됨 => 막기

          응용 2. 따봉버튼 수정(추가, 삭제)

          응용 3. 날짜 state로 만들어서 현재 날짜로 저장
        */
     ...
     <button onClick={() => {
      ...
      { // 자스쓰려면 이거 열기
        modal == true ? <Modal title={title} b={b} color={'skyblue'} a={a}/> : null
      }
      ...
      {/* 
        * 리액트에서 동적인 UI(원래 없었는데 생기는거) 만드는 STEP
        ...
        2. UI의 현재상태를 state 로 저장해두기
        3. state의 상태에 따라서 UI가 어떻게 보일지 조건문 
        
        ** 챌린지 : 글제목을 클릭하면 모달창이 보이게
        ** 힌트 : 글제목 누르면 html 보이게해주세요(x)
                  글제목 누르면 state의 값을 변경해주세요

        ** 챌린지2탄 : 글제목 한번 더 누르면 모달창이 닫히고          
      */}
...
* 컴포넌트의 단점
- 한 function 안에 있는 변수를 다른 function 호출 할 수 없음
...
function Modal(props){
  ...
  return(
    ...
    <button onClick={()=>{
      // 버튼 누르면 첫번째 글 수정 
      let copy = [...props.a]; // ['여름 코트 추천', '역삼우동 맛집', '리액트 독학']
      copy[0] = '겨울 코트 추천';
      props.b(copy)
...
function Nav(){
  return(
        ...
        ** JSX 문법
        1. 리액트 에서는 class 말고 className 이라고 쓰장 
        => 왜 리엑트에서는 JSX 문법을 사용할까? 
        원래 : 리액트 <div> 만들고자하면 
        ...
        => ex) font-size : 뺄샘 처럼 인식 => fontSize 
        ...
        ** 자주 변경 되는 데이터 state에 저장해놨다가 html {데이터바인딩}
        ...
        // 오늘 배운 내용 정리
        1. 클릭시 뭔가 실행하고 싶으면 onClick={함수} 이렇게 이벤트 핸들러를 담아서 사용
        2. state를 변경하시려면 state 변경 함수를 꼭 이용해라.
        => state변경 함수는 ()안에 입력한걸로 기존 state를 갈아치워준당.
        => 중요하니 외워야한다!!!

        글제목변경('가을 코트 추천') => 이거왜안되나요?
        기존 state를 갈아 치워주기 때문에
        ['여름 코트 추천', '역삼 우동 맛집', '리액트 독학']
        글제목변경(['가을 코트 추천', '역삼 우동 맛집', '리액트 독학'])


        [...복사할데이터] => 독립적인 복사본을 생성 해줌 (깊은 복사)
        => 리액트에서 array, object state를 수정하고 싶으면
        독립적인 카피본을 만들어서 수정
        [...기존state] 

        ** 응용문제 : 가나다순 정렬버튼 => 
        저는 array 정렬하는법 모르는디요.... -> 당연히 검색해바 제바루../
